...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("C:\Development\learn\darkflow-master\lib\classifications.txt", np.float32)       
    ...
    try:
        npaFlattenedImages = np.loadtxt('C:\Development\learn\darkflow-master\lib\\flattened_images.txt', np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    if recognizePlate.showSteps == True:
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    for currentChar in listOfMatchingChars:                                         # for each char in plate
        ...
        cv2.rectangle(imgThreshColor, pt1, pt2, recognizePlate.SCALAR_GREEN, 2)           # draw green box around the char
